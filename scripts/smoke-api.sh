...
# /api/v1/enrollments and /api/v1/fleet want a dashboard session.
printf '2/6 log in... '
...
printf '3/6 create enrollment... '
ENROLLMENT_JSON="$(curl -fsS -b "$COOKIE_JAR" -X POST "${API_URL}/api/v1/enrollments" \
  -H 'Content-Type: application/json' \
...
printf '5/6 send camera telemetry... '
...
printf '6/6 verify live fleet... '
SOURCE="$(curl -fsS -b "$COOKIE_JAR" "${API_URL}/api/v1/fleet" | python3 -c 'import json,sys; print(json.load(sys.stdin)["source"])')"
if [[ "$SOURCE" != "live" ]]; then
